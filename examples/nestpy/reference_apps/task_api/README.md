...
This is a composed Nestpy v1 example, not a production task service. It shows:
...
```text
uv add 'nestpy[cli]'
nestpy run examples.nestpy.reference_apps.task_api.app:create_application
```

For direct ASGI hosting, use the exported wrapper:

```text
uvicorn examples.nestpy.reference_apps.task_api.app:application
```

Create a task:

```powershell
curl.exe -X POST "http://127.0.0.1:8000/tasks" -H "content-type: application/json" -H "x-task-write: allow" -d "{\"title\":\"Read Nestpy guides\"}"
```
...
authorization policy are application concerns outside Nestpy v1.
