...
This larger reference project combines `cqrs-core`, `cqrs-event-sourcing`,
`nestpy-cqrs`, `nestpy-cqrs-event-sourcing`, and Nestpy HTTP/testing in one
layered application.
...
event_sourcing/
    ...
    app.py               Nestpy composition root
...
6. Nestpy returns the candidate only after confirmed commit and scope cleanup.
...
```text
uv run pytest examples/nestpy/cqrs/event_sourcing -q
uv run uvicorn examples.nestpy.cqrs.event_sourcing.app:application
```
