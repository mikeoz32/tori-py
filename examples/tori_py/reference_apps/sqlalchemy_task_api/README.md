# ToriPy SQLAlchemy Task API

This reference application uses ToriPy, `tori-py-sqlalchemy`, SQLAlchemy ORM,
...
Run through the ToriPy CLI:

```text
uv run tori-py run examples.tori_py.reference_apps.sqlalchemy_task_api.app:create_application
```
...
- `SqlAlchemyModule.for_root_async()` receives `TaskApiSettings` through ToriPy
...
- ToriPy has no migration, database health-check, response-schema, or OpenAPI
