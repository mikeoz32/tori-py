...
ToriPy is a Python application framework for applications that need explicit
...
ToriPy does not include persistence, migrations, brokers, CQRS, authentication,
...
See [Why ToriPy](why-tori-py.md) for the design boundaries.
...
`tori_py.http` owns HTTP route plans, execution contexts, guards, pipes,
interceptors, filters, errors, and validation. `tori_py.starlette` only connects
