...
| Server-rendered interactive pages | `tori-py-liveview` / `tori_py_liveview` | Page and component state should remain on the server with a thin browser runtime | Depends on Tori Py and Starlette. Protocol v2 covers pages, structural diffs, and connection-local stateful components; nested components, streams, uploads, and `send_info` are not implemented. |
