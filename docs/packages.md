...
| Server-rendered interactive pages | `tori-py-liveview` / `tori_py_liveview` | Page, component, and collection state should remain server-directed with a thin browser runtime | Depends on Tori Py and Starlette. Protocol v2 covers pages, structural diffs, connection-local stateful components, browser-owned streams, and serialized `send_info`; nested components, uploads, navigation, and server hook/reply APIs are not implemented. |
| Styled LiveView foundation components | `tori-py-liveview-ui` / `tori_py_liveview_ui` | A LiveView page needs the local Tori UI stylesheet and small stateless template components | Depends on Tori Py and LiveView. It supplies closed-variant `button`, `badge`, `alert`, `card`, `stack`, and `grid`, not forms, modals, navigation, JavaScript widgets, arbitrary attribute/class injection, or component state. |
