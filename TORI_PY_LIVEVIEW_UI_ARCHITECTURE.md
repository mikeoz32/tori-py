...
component, stream, and explicitly trusted `SafeHtml` values compose according to
LiveView's Phoenix render-tree contract. `stack` and `grid` use `fragment()` and
...
root, and official Phoenix browser client. `ui_theme()` defaults to `"auto"` and
may return only `"auto"`, `"light"`, or `"dark"`; any other value fails
rendering. A custom `LiveView.render_document()` can call `stylesheet_link()`
itself instead.
...
`--tori-ui-*` custom properties define surfaces, text, lines, tones, shadows, and
font stacks. Applications may override those properties in their own CSS. Font
