...
Mechanical import-order edits under the existing Nestpy microservices examples and
tests are intentionally retained but must be isolated in a separate later commit;
no persistent-streams changes should be mixed into those files.
...
2. Keep core `PersistentLog` conformance separate from Nestpy lifecycle/execution
   conformance.
...
- Run Nestpy lifecycle/execution conformance against the configured factory/runtime.
