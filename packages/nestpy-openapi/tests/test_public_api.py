...
def test_public_facade_is_the_exact_allowlist() -> None:
    assert set(nestpy_openapi.__all__) == PUBLIC_API
    assert nestpy_openapi.OpenApiModule.__module__ == "nestpy_openapi.module"


def test_error_hierarchy_is_typed_and_has_stable_codes() -> None:
    assert issubclass(OpenApiError, NestpyError)
...
def test_package_artifacts_and_runtime_dependencies_are_exact() -> None:
    ...
    assert (package_root / "src/nestpy_openapi/py.typed").is_file()
    ...
    assert dependencies == {"msgspec", "nestpy", "starlette"}
