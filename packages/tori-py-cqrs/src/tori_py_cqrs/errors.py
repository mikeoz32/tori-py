"""Errors raised by the ToriPy CQRS integration."""

import asyncio

from tori_py_cqrs_core import CqrsError


class ToriPyCqrsError(CqrsError):
    """Base error for integration-specific failures."""


class CqrsConfigurationError(ToriPyCqrsError):
    """Raised when explicit ToriPy CQRS configuration is invalid."""


class CqrsLifecycleError(ToriPyCqrsError):
    """Raised when coordinated CQRS startup or shutdown fails."""


class CqrsPipelineStateError(ToriPyCqrsError):
    """Raised when CQRS invocation pipeline state is used incorrectly."""


class CqrsHandlerExitError(ToriPyCqrsError):
