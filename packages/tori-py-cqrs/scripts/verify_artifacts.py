"""Smoke-test tori-py-cqrs artifacts with local dependency artifacts."""
...
assert Path(tori_py_cqrs.__file__).with_name("py.typed").is_file()
asyncio.run(smoke())
"""
...
def main() -> None:
    ...
    artifact_sets = (
        (
            ...
            _one(dist, "tori_py-*.whl"),
            _one(dist, "tori_py_cqrs-*.whl"),
        ),
        (
            _one(dist, "tori_py_cqrs_core-*.tar.gz"),
            _one(dist, "tori_py-*.tar.gz"),
            _one(dist, "tori_py_cqrs-*.tar.gz"),
        ),
    )
