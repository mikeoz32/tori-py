"""Smoke-test cqrs-event-sourcing wheel and source distributions."""
...
assert Path(cqrs_event_sourcing.__file__).with_name("py.typed").is_file()
runtime_requirements = requires("cqrs-event-sourcing") or []
...
assert normalized_name == "cqrs-core"
asyncio.run(smoke())
"""
...
def main() -> None:
    ...
    artifact_sets = (
        (
            ...
            _one(dist, "cqrs_event_sourcing-*.whl"),
        ),
        (
            _one(dist, "cqrs_core-*.tar.gz"),
            _one(dist, "cqrs_event_sourcing-*.tar.gz"),
        ),
    )
