...
import pytest
from cqrs_core import Command, CommandHandler, Query, QueryHandler
...
from nestpy import Inject, ModuleSpec
...
def test_public_api_allowlist_and_type_marker() -> None:
    assert set(nestpy_cqrs_event_sourcing.__all__) == {
    ...
    assert (package_root / "src/nestpy_cqrs_event_sourcing/py.typed").is_file()
...
assert 'EventSourcingUnitOfWork' not in nestpy_cqrs_event_sourcing.__all__
"""
    ...
    source = Path(__file__).parents[1] / "src/nestpy_cqrs_event_sourcing"
    dependency_roots = {
        "cqrs_core",
        "cqrs_event_sourcing",
        "nestpy",
    ...
    for path in source.glob("*.py"):
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ImportFrom) and node.module:
                ...
                if root in dependency_roots - {"nestpy_cqrs_event_sourcing"}:
