"""Smoke-test nestpy-cqrs-event-sourcing wheel and source distributions."""
...
from cqrs_core import Command, CommandBus, Event
...
from nestpy import ClassProvider, Scope, module
...
assert Path(nestpy_cqrs_event_sourcing.__file__).with_name("py.typed").is_file()
names = {
    re.sub(r"[-_.]+", "-", re.match(r"[A-Za-z0-9_.-]+", item).group().lower())
    for item in (requires("nestpy-cqrs-event-sourcing") or [])
}
assert names == {"cqrs-core", "cqrs-event-sourcing", "nestpy", "nestpy-cqrs"}
...
def _verify_contents(artifact: Path) -> None:
    if artifact.suffix == ".whl":
        ...
        assert "nestpy_cqrs_event_sourcing/py.typed" in names
        assert not any("/tests/" in name for name in names)
        return
    with tarfile.open(artifact, "r:gz") as archive:
        names = set(archive.getnames())
    assert any(
        name.endswith("/src/nestpy_cqrs_event_sourcing/py.typed") for name in names
    )
...
def main() -> None:
    ...
    dependency_names = ("cqrs_core", "cqrs_event_sourcing", "nestpy", "nestpy_cqrs")
    for suffix in ("*.whl", "*.tar.gz"):
        artifacts = [_one(dist, f"{name}-{suffix}") for name in dependency_names]
        package_artifact = _one(dist, f"nestpy_cqrs_event_sourcing-{suffix}")
