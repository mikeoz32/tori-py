...
@pytest.mark.asyncio
async def test_custom_and_default_repositories_share_ambient_transaction(
    tmp_path: Path,
) -> None:
    engine, entities = await _create_manager(tmp_path / "ambient.db")
    ...
    try:
        ...
        with pytest.raises(RuntimeError, match="abort repository transaction"):
            async with entities.transaction() as transaction:
                default = Repository(Item, entities)
                assert transaction is entities
                created = await custom.add(Item(name="inside", category="temporary"))
                assert await default.get(created.id) is created
                assert await custom.get(created.id, with_for_update=True) is created
                assert (
                    await custom.find_one(
                        Item.id == created.id,
                        with_for_update=True,
                    )
                    is created
                )
                assert await custom.count(Item.category == "temporary") == 1
                assert await custom.exists(Item.name == "inside")
                assert [item.name for item in await custom.find_named("in")] == [
                    "inside"
                ]
                raise failure
...
async def test_repository_validates_entities_pagination_and_lock_lifetime(
    ...
    engine, entities = await _create_manager(tmp_path / "validation.db")
    items = Repository(Item, entities)
    try:
...
async def test_single_result_queries_apply_a_two_row_limit(tmp_path: Path) -> None:
    engine, entities = await _create_manager(tmp_path / "single-result.db")
    items = Repository(Item, entities)
