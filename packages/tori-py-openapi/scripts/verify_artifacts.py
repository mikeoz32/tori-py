"""Smoke-test tori-py-openapi wheel and source distributions."""
...
from tori_py_openapi import OpenApiInfo, OpenApiOptions, SwaggerUiOptions
...
actual = set()
for requirement in requires("tori-py-openapi") or ():
    match = re.match(r"[A-Za-z0-9_.-]+", requirement)
    assert match is not None
    actual.add(re.sub(r"[-_.]+", "-", match.group().lower()))
assert actual == expected
assert Path(tori_py_openapi.__file__).with_name("py.typed").is_file()
...
def main() -> None:
    ...
    artifact_sets = (
        (
            ...
            _one(dist, "tori_py_openapi-*.whl"),
        ),
        (
            _one(dist, "tori_py-0.1.0.tar.gz"),
            _one(dist, "tori_py_openapi-*.tar.gz"),
        ),
    )
