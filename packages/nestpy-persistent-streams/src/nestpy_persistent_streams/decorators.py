...
from nestpy import Inject, MetadataKey, Reflector, metadata
...
class StreamInject(Inject):
    """Bind a normal Nestpy provider from the handler work scope."""


_HANDLER_KEY: MetadataKey[StreamHandlerMetadata] = MetadataKey(
    "nestpy.persistent_streams.handler"
)
_PUBLISH_KEY: MetadataKey[StreamPublishMetadata] = MetadataKey(
    "nestpy.persistent_streams.publish"
)
