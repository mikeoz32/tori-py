...
`tori-py-persistent-streams` connects ToriPy modules and execution scopes to the
framework-neutral `tori-py-persistent-streams-core` contracts. It does not contain a broker
driver and does not bridge CQRS events or `tori-py-microservices` handlers.
...
from tori_py import Inject, controller, module
...
The root is always global and composes adapter modules through normal ToriPy imports.
...
`bindings` and `publishers` are always static because ToriPy must know provider
...
- A record checkpoints only after decoding, the complete ToriPy pipeline, handler
...
and save uncertainty retain their `tori-py-persistent-streams-core` meanings. The integration
...
`StreamAdapterFactory` Protocol class as its canonical ToriPy token. Its
`for_root()` and `for_root_async()` methods return a `DeferredModule` directly.
The factory receives resolved immutable bindings and
returns a public `tori_py_persistent_streams_core.PersistentStreamAdapter`. Its `start()`
...
outcomes, and close semantics. It must not discover ToriPy controllers or invoke
handler pipelines.

Run core `PersistentLog` conformance against the log implementation, then run
the ToriPy package lifecycle/execution tests against the configured adapter.
Passing one suite does not replace the other. The in-memory module under
`tori_py_persistent_streams.testing` is the reference composition, not a durable
production adapter.
...
conformance boundary but are not lifecycle-complete ToriPy adapters.
Core `PersistentLog` conformance and ToriPy lifecycle tests remain separate.
