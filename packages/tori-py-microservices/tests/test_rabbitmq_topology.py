...
from tori_py_microservices.rabbitmq import (
    QueueDeclaration,
    RabbitMqChannelRole,
...
from tori_py_microservices.rabbitmq import connection as rabbitmq_connection
from tori_py_microservices.rabbitmq import publisher as rabbitmq_publisher
from tori_py_microservices.rabbitmq.connection import RabbitMqChannels
from tori_py_microservices.rabbitmq.topology import retry_exchange_name
...
def test_options_redact_credentials_and_validate_defaults() -> None:
    ...
    assert "name=tori-py-microservices" in options.connection_url
...
def test_rpc_topology_uses_one_wildcard_service_queue() -> None:
    ...
    assert topology.exchanges[0].name == "tori_py.rpc"
    assert topology.queues[0].name == "tori_py.rpc.kinker.members.v1"
    assert topology.bindings[0].routing_key == "kinker.members.v1.*"
    assert dict(topology.queues[0].arguments) == {
        "x-queue-type": "quorum",
        "x-delivery-limit": 5,
        "x-dead-letter-exchange": "tori_py.dead-letter",
    ...
    assert dict(retry.arguments) == {
        ...
        "x-message-ttl": 1_000,
        "x-dead-letter-exchange": "tori_py.rpc",
        "x-max-length": 10_000,
...
def test_long_retry_exchange_name_is_stable_and_broker_safe() -> None:
    ...
    assert first == second
    assert first.startswith("tori_py.retry.")
...
async def test_server_consumes_and_manually_settles_incoming_messages() -> None:
    ...
    assert retried[0].native == (
        "tori_py.rpc.kinker.members.v1.retry",
        "kinker.members.v1.get",
    )
    assert retried[0].headers["x-tori_py-retry-count"] == 2
...
async def test_client_publishes_rpc_and_events_and_correlates_replies(
    ...
    assert published == [
        ("tori_py.rpc", rpc_publication.routing_key, True),
...
async def _return_queue(queue, topology=None):
    ...
    return {"tori_py.rpc.kinker.members.v1": queue}
