...
from tori_py_microservices import (  # noqa: E402
    Context,
    EncodedDelivery,
    EventContext,
    EventDispatcher,
...
from tori_py_microservices.errors import (  # noqa: E402
    RabbitMqConnectionError,
    RpcOutcomeUnknownError,
    RpcTimeoutError,
    TransportIndeterminateError,
)
from tori_py_microservices.rabbitmq import compile_event_topology  # noqa: E402
from tori_py_microservices.testing import assert_transport_conformance  # noqa: E402
...
def rabbitmq_url(docker_services, docker_ip: str) -> str:
    ...
    url = f"amqp://tori_py:tori_py@{docker_ip}:{port}/"
...
def _load_management_collection(url: str, resource: str) -> list[dict[str, object]]:
    credentials = base64.b64encode(b"tori_py:tori_py").decode("ascii")
    request = Request(
        f"{url}/{resource}/%2F",
...
async def test_rabbitmq_event_mode_cardinality_and_exact_queue_count(
    ...
    try:
        ...
        queue_prefix = (
            f"tori_py.event.{source.label}.{event.event}.v{event.schema_version}"
        )
        durable_primaries = {
        ...
        expected_bindings = (
            ...
            | {
                ("tori_py.dead-letter", f"{name}.dead-letter", name)
...
async def test_reliable_broadcast_rejects_duplicate_and_retains_across_restart(
    ...
    try:
        ...
        if len(retry_exchange.encode("utf-8")) > 127:
            retry_exchange = (
                f"tori_py.retry.{sha256(primary_queue.encode('utf-8')).hexdigest()}"
            )
        expected_queue_names = {
            primary_queue,
