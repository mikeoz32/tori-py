...
from tori_py import DeferredModule, FactoryProvider, Inject, ModuleSpec, ValueProvider
...
from tori_py_microservices.rabbitmq.server import RabbitMqServerTransport
...
class RabbitMqTransport:
    """Reference one configured RabbitMQ root by its ToriPy key."""
...
def rabbitmq_root_token(key: str) -> str:
    _validate_key(key)
    return f"tori_py.rabbitmq.root.{key}"


def rabbitmq_manager_token(key: str = "default") -> str:
    _validate_key(key)
    return f"tori_py.rabbitmq.manager.{key}"


def rabbitmq_server_factory_token(key: str = "default") -> str:
    _validate_key(key)
    return f"tori_py.rabbitmq.server_factory.{key}"


def rabbitmq_client_factory_token(key: str = "default") -> str:
    _validate_key(key)
    return f"tori_py.rabbitmq.client_factory.{key}"
