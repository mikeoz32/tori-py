...
from tori_py_microservices.compiler import (
    compile_discovered_service_handlers,
    qualify_pipeline_plan,
)
from tori_py_microservices.decorators import EventDispatchMode
...
from tori_py_microservices.plans import ServiceHandlerRegistry
    ...
    async def _prepare_locked(self) -> None:
        ...
        if _has_provider_binding(global_pipeline):
            if self._work_scopes is None:
                raise TransportStateError(
                    "global provider bindings require a ToriPy work-scope factory"
                )
