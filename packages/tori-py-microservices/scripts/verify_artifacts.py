"""Smoke-test tori-py-microservices wheel and source distributions."""
...
def _wheel_inventory() -> set[str]:
    package = {f"tori_py_microservices/{name}" for name in PACKAGE_MODULES}
    package.update(
        f"tori_py_microservices/rabbitmq/{name}" for name in RABBITMQ_MODULES
    )
    return {
        "tori_py_microservices/",
        "tori_py_microservices/rabbitmq/",
        *package,
        "tori_py_microservices-0.1.0.dist-info/",
        "tori_py_microservices-0.1.0.dist-info/METADATA",
        "tori_py_microservices-0.1.0.dist-info/RECORD",
        "tori_py_microservices-0.1.0.dist-info/WHEEL",
    }


def _sdist_inventory() -> set[str]:
    root = "tori_py_microservices-0.1.0"
    package = {f"{root}/src/tori_py_microservices/{name}" for name in PACKAGE_MODULES}
    package.update(
        f"{root}/src/tori_py_microservices/rabbitmq/{name}" for name in RABBITMQ_MODULES
    )
    return {
        root,
        f"{root}/PKG-INFO",
        ...
        f"{root}/src",
        f"{root}/src/tori_py_microservices",
        f"{root}/src/tori_py_microservices/rabbitmq",
        *package,
    }
...
def _read_metadata(artifact: Path) -> bytes:
    if artifact.suffix == ".whl":
        with zipfile.ZipFile(artifact) as archive:
            return archive.read("tori_py_microservices-0.1.0.dist-info/METADATA")
    with tarfile.open(artifact) as archive:
        member = archive.extractfile("tori_py_microservices-0.1.0/PKG-INFO")
        if member is None:
...
def _assert_metadata(artifact: Path) -> None:
    ...
    assert metadata["Name"] == "tori-py-microservices"
...
import tori_py_microservices
import tori_py_microservices.rabbitmq as rabbitmq

assert set(tori_py_microservices.__all__) == {
    "EventEnvelope",
    "EventDispatcher",
    "EventIdentity",
    "EventDispatchMode",
...
assert all(
    hasattr(tori_py_microservices, name) for name in tori_py_microservices.__all__
)
...
def main() -> None:
    ...
    artifact_sets = (
        (
            _one(dist, "tori_py-0.1.0-*.whl"),
            _one(dist, "tori_py_microservices-*.whl"),
        ),
        (
            _one(dist, "tori_py-0.1.0.tar.gz"),
            _one(dist, "tori_py_microservices-*.tar.gz"),
        ),
    )
    ...
    for artifacts in artifact_sets:
        ...
        if len(sys.argv) == 3:
            ...
            requirement = (
                f"tori-py-microservices[rabbitmq] @ {rabbitmq.resolve().as_uri()}"
            )
            command = [
                "uv",
                "run",
