...
def module(
    *,
    ...
    def decorate(target: type[object]) -> type[object]:
        if "__tori_py_module_metadata__" in target.__dict__:
            raise BootstrapError(
                "module metadata is already declared on this target",
                code="module.duplicate_metadata",
            )
        type.__setattr__(target, "__tori_py_module_metadata__", metadata)
        return target
...
def get_module_metadata(target: type[object]) -> ModuleMetadata | None:
    ...
    metadata = target.__dict__.get("__tori_py_module_metadata__")
