...
def header(name: str, value: str) -> Any:
    """Attach one static header to a ToriPy-encoded route response."""
    ...
    def decorate(target: Any) -> Any:
        ...
        target.__tori_py_response_header_metadata__ = (*existing, metadata)
        return target
...
def get_response_header_metadata(target: Any) -> tuple[ResponseHeaderMetadata, ...]:
    ...
    metadata = getattr(target, "__dict__", {}).get(
        "__tori_py_response_header_metadata__", ()
    )
