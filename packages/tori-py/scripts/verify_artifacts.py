"""Smoke-test ToriPy wheel and source distributions in isolated uv runs."""
...
from tori_py import NestApplication, Query, controller, get, injectable, module
...
def main() -> None:
    ...
    artifacts = sorted(dist.glob("tori_py-*.whl")) + sorted(
        dist.glob("tori_py-*.tar.gz")
    )
    if not artifacts:
        raise SystemExit(f"no ToriPy artifacts found in {dist}")
    uv = "uv"
