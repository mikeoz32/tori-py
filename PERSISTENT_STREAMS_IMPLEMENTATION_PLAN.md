...
gates pass. Repository-wide gates retain unrelated baseline failures documented
in the implementation verification results.

Architecture: [`PERSISTENT_STREAMS_ARCHITECTURE.md`](PERSISTENT_STREAMS_ARCHITECTURE.md).

The executable phase map and cross-phase invariants are recorded in
[`spec/persistent-streams/README.md`](spec/persistent-streams/README.md). This
...
- distribution: `persistent-streams`;
- import package: `persistent_streams`;
- runtime dependencies: Python standard library only;
- no dependency on another workspace distribution.

The package MUST remain independent of CQRS, Nestpy, RabbitMQ, SQLAlchemy,
PostgreSQL, msgspec, Pydantic, and Kinker application code. The in-memory
...
2. Add `packages/persistent-streams` to the `uv` workspace.
...
```text
uv run pytest packages/persistent-streams/tests
uv run ruff check .
uv run ruff format --check .
uv run ty check packages/persistent-streams/src packages/persistent-streams/tests
uv build --package persistent-streams
uv run pytest
```
...
- No CQRS, Nestpy, SQLAlchemy, Pydantic, or msgspec dependency.
