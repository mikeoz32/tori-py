# Nestpy Microservices Architecture

## 1. Status and Purpose

This document defines the accepted architecture for the optional
`nestpy-microservices` integration. It combines a NestJS-inspired programming
...
Executable phase contracts live under `spec/nestpy-microservices/`.

The integration provides:

- declarative RPC and event handlers on ordinary Nestpy controllers;
...
The integration does not turn the Kinker modular monolith into independently
deployed services. It supplies a reusable capability that can be adopted where
deployment boundaries and distributed failure semantics are explicitly accepted.
...
nestpy-microservices[rabbitmq]
  -> aio-pika >= 10, < 11
```

The base package MUST import without importing `aio_pika`. RabbitMQ symbols live
under `nestpy_microservices.rabbitmq` and fail with an actionable optional-extra
message when the extra is absent.

`nestpy` MUST NOT import `nestpy-microservices`, RabbitMQ, or `aio-pika`.
`nestpy-microservices` MUST NOT import Starlette, SQLAlchemy, CQRS, event
sourcing, Alembic, or Kinker application modules.

The package layout is expected to converge on:

```text
packages/nestpy-microservices/
  src/nestpy_microservices/
    __init__.py
    bindings.py
    client.py
    codecs.py
...
may expose any number of controllers across any number of normal Nestpy modules.
...
The handler compiler injects Nestpy `DiscoveryService` and calls
...
matches current Nestpy HTTP mapping behavior and avoids accidental inherited
published interfaces.

Discovery performs no package import, filesystem scan, subclass enumeration, or
runtime provider registration. A class that is not an explicit Nestpy controller
...
- `Annotated[T, Inject(token)]` resolves a normal Nestpy provider.
...
RPC and event executors reuse Nestpy's driver-neutral `Guard`, `Pipe`,
...
- one fresh Nestpy work scope per delivery attempt;
...
Controller instances remain Nestpy singleton controllers. Message-specific
...
pipeline components, or own Nestpy work scopes.
...
```text
queue:       nestpy.rpc.kinker.members.v1
...
exact grammar and always publish through `nestpy.rpc`.
...
```text
exchange: nestpy.events.<namespace>.<source_service>.v<contract_version>
```
...
Server and client runtimes are independent lifecycle-managed Nestpy providers,
...
1. compile the Nestpy module/provider graph;
...
6. let each server/client runtime declare and verify its topology without
   admitting messages;
7. let the Nestpy kernel open work-scope admission;
...
5. allow their Nestpy work scopes to finish and settle;
...
## 35. CQRS, Persistence, and Kinker Boundaries

`nestpy-cqrs` remains an in-process application bus. Its existing transport
...
Reliable Kinker integration events require:
...
Current linearizable cross-context authorization in the Kinker modular monolith
cannot be replaced by eventual RabbitMQ events. Service extraction requires the
alternative consistency protocol required by `spec/kinker/cross-context-integration.md`.
...
12. keep Nestpy core, CQRS, persistence, and application boundaries intact;
