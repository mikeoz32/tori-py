...
[`spec/cqrs-event-sourcing/README.md`](spec/cqrs-event-sourcing/README.md).
...
of the completed first `cqrs-core` slice.
...
- distribution: `cqrs-event-sourcing`;
- import package: `cqrs_event_sourcing`;
- runtime dependency: `cqrs-core` only;
...
cqrs-fastapi         -> FastAPI, cqrs-core
```

There is no reverse import from `cqrs-core`, `nestpy`, or `nestpy-cqrs` into the
event-sourcing package. `AggregateRoot` and `EventStore` do not belong in
`cqrs-core`: CQRS can be used without event sourcing, and the existing bus and
transport contracts must remain persistence-agnostic.

The package must not depend on Nestpy, FastAPI, Pydantic, msgspec, SQLAlchemy,
...
- **Domain event**: an immutable `cqrs_core.Event` describing a domain fact.
...
Persisted event type identity is not `cqrs_core.identity.message_type_for()`.
...
The existing `cqrs-core.EventBus` remains suitable for non-durable, in-process
notifications. Metadata-preserving publication through that bus requires a
separate `cqrs-core` specification rather than a private API dependency.
...
```text
uv run pytest packages/cqrs-event-sourcing/tests
uv run ruff check .
uv run ruff format --check .
uv run ty check packages/cqrs-event-sourcing/src packages/cqrs-event-sourcing/tests
uv build --package cqrs-event-sourcing
uv run pytest
```
...
- No automatic publication through `cqrs-core.EventBus`.
...
- No Nestpy/FastAPI integration module in this package.
