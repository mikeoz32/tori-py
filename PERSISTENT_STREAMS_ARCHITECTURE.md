...
`spec/persistent-streams/phase-ps0-*.md` through `phase-ps4-*.md`.

## 1. Purpose

`persistent-streams` is a framework-neutral abstraction for an append-only,
...
- one standalone `persistent-streams` distribution;
- a Python 3.14 import package named `persistent_streams`;
...
- Nestpy modules, dependency injection, controllers, or lifecycle integration;
...
It has no dependency on `cqrs-core`, `cqrs-event-sourcing`, Nestpy, RabbitMQ,
SQLAlchemy, msgspec, or any Kinker application package. Those packages also gain
