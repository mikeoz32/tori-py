...
## Tori Py LiveView UI
- The optional styled foundation is governed by `TORI_PY_LIVEVIEW_UI_ARCHITECTURE.md`, `TORI_PY_LIVEVIEW_UI_IMPLEMENTATION_PLAN.md`, and `spec/tori-py-liveview-ui/README.md`.
- Keep it separately installable and stateless. Components return typed templates and reuse public LiveView rendering and Opal contracts; they do not own state or add a browser runtime.
- Keep assets local and content-addressed, selectors prefixed, themes token-based, and behavior accessible without CDN, Node, external fonts, arbitrary attribute injection, or a global reset.
