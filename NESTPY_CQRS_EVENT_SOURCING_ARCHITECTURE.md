# Nestpy CQRS Event-Sourcing Integration Architecture

Status: NES0-NES5 implemented. Executable requirements are split into
`spec/nestpy-cqrs-event-sourcing/phase-nes0-*.md` through `phase-nes5-*.md`.

## 1. Purpose

`nestpy-cqrs-event-sourcing` is the optional integration layer that makes
`cqrs-event-sourcing` feel native inside a Nestpy CQRS application. It removes
...
- a standalone `nestpy-cqrs-event-sourcing` distribution;
- Nestpy dynamic-module configuration with `for_root()` and `for_feature()`;
...
- a Nestpy-style CQRS invocation-interceptor extension point;
...
- exact Nestpy module visibility, provider scope, resource ownership, keyed
...
```text
nestpy-cqrs-event-sourcing -> nestpy-cqrs, nestpy,
                              cqrs-event-sourcing, cqrs-core
nestpy-cqrs                -> nestpy, cqrs-core
cqrs-event-sourcing        -> cqrs-core
nestpy                     -> no CQRS or event-sourcing package
cqrs-core                  -> no framework or event-sourcing package
```

Distribution and import names are:

```text
distribution: nestpy-cqrs-event-sourcing
import:       nestpy_cqrs_event_sourcing
```
...
classification remain owned by `cqrs-event-sourcing`. Handler routing,
discovery, transports, and bus lifecycle remain owned by `nestpy-cqrs` and
`cqrs-core`. Provider visibility, scopes, resources, and application lifecycle
remain owned by Nestpy.
...
### 5.1 Nestpy resource unwinding

Nestpy request-resource cleanup MUST become exception-aware. The current
...
remaining exits run. Nestpy then raises `ScopeFinalizationError`, carrying the
original ordinary body error when present plus an ordered tuple of cleanup
errors. If the primary failure is `CancelledError`, Nestpy raises
...
### 5.2 Nestpy CQRS invocation interceptors

`nestpy-cqrs` MUST expose a driver-neutral invocation pipeline. Its shape follows
Nestpy interceptors without reusing HTTP `PipelineResult`:
...
`CqrsInvocationContext` MUST implement Nestpy `ExecutionContext` and expose:

- `execution_kind == "cqrs"`;
- `application_id` from the Nestpy application kernel;
...
`nestpy-cqrs` MUST preserve an invocation completion record across work-scope
...
`cqrs-core` MUST reject same-bus nested command execution before transport
...
The configured `store` is a visible Nestpy provider token, not an object opened
...
The UoW is created inside the coordinator and is never a Nestpy provider or DI
...
Nestpy-native equivalent of NestJS dynamic-module metadata identity: independent
...
decorator on domain aggregate classes. Keeping aggregates free of Nestpy imports
...
immutable direct metadata through Nestpy reflection. It supplies
...
`for_feature()` reads this metadata through Nestpy `Reflector` while creating the
...
`aggregate_repository()` is deliberately overloaded for the two related Nestpy
declaration sites:
...
it returns the standard Nestpy `Inject(MemberRepository)` marker. It is not a
second injection system. Passing an undecorated class or mixing the two call
forms is an immediate configuration error.

The repository class itself is the provider token. Nestpy module qualification
...
Undecorated command handlers retain current `nestpy-cqrs` behavior. Manual UoW
injection is not a compatibility mode offered by this package; an application
that requires manual control can use `cqrs-event-sourcing` directly.
...
CommandBus transport delivery
  ...
  -> open one fresh Nestpy CQRS work scope
...
The adapter owns no worker or independent shutdown hook. `nestpy-cqrs` stops
command intake and drains accepted work while Nestpy work scopes remain open. A
...
- A singleton handler cannot inject a request-scoped repository; Nestpy's normal
...
`cqrs-event-sourcing` types when no secondary finalization failure requires an
...
only its feature descriptor. Root descriptors still follow Nestpy's exact
...
`spec/nestpy-cqrs-event-sourcing/README.md`.
