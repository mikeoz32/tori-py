# ToriPy SQLAlchemy Architecture

## 1. Status and Scope

This document defines the `tori-py-sqlalchemy` integration. The distribution
connects ToriPy singleton lifecycle and dependency injection to SQLAlchemy's
...
`tori_py` MUST NOT import SQLAlchemy or this integration. The integration MUST
...
singleton ToriPy provider. Factory dependencies use annotations and
...
```text
tori_py_sqlalchemy:<key>:engine
tori_py_sqlalchemy:<key>:session_factory
tori_py_sqlalchemy:<key>:entity_manager
```
...
`inject_repository(Entity, key=...)` is a normal ToriPy `Inject` marker over a
