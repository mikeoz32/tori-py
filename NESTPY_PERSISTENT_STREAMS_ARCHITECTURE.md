# Nestpy Persistent Streams Architecture

## 1. Status and Purpose

This document defines the accepted architecture for the optional
`nestpy-persistent-streams` integration. It connects the framework-neutral
`persistent-streams` contracts to Nestpy modules, discovery, dependency
...
Executable phase contracts live under `spec/nestpy-persistent-streams/`.
...
- normal Nestpy guards, pipes, interceptors, filters, and work scopes;
...
`nestpy` and `persistent-streams` MUST NOT import this integration. This package
MUST NOT import RabbitMQ, `rstream`, `nestpy-microservices`, CQRS, event
sourcing, SQLAlchemy, Starlette, Alembic, or Kinker application modules.
...
contracts, Nestpy composition, and Nestpy execution.
...
Nestpy provider for `PersistentStreamsRuntimeOptions`. The factory may be
...
Handlers are ordinary methods on explicitly registered Nestpy controllers:
...
- `StreamInject(token)` for normal Nestpy provider resolution.

These markers and `@stream_handler` are independent of
`nestpy-microservices`. Neither package imports, aliases, recognizes, or
...
Each decoded record executes through one fresh exact-owner Nestpy work scope:
...
The integration reuses Nestpy's transport-neutral `Guard`, `Pipe`,
...
application-owned resources and can use normal Nestpy DI. The runtime does not
...
Nestpy pipelines, owns work scopes, or advances a checkpoint before the
...
1. compile the Nestpy graph and enforce one global stream root;
...
8. open Nestpy work-scope admission;
...
Native adapter credentials and TLS policy remain adapter concerns. The Nestpy
...
`nestpy-microservices` remains responsible for RPC and queue-event semantics.
...
Concrete logs run core `PersistentLog` conformance. Nestpy adapters separately
