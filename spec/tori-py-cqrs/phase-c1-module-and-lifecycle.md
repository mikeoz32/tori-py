...
Provide a dynamic ToriPy module whose CQRS handlers use ToriPy scopes and whose
...
- `CqrsModule.for_root()` accepts explicit imports, bindings, options, key, and
  global visibility.
- Every binding compiles a private alias to a visible ToriPy provider token.
- One work scope is opened per command, query, or individual event-handler
  invocation through `WorkScopeFactory.run()`.
- Singleton, request, and transient behavior comes only from ToriPy provider
  declarations.
