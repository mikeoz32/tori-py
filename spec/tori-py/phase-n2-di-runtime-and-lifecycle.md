...
the reserved grace. If a hook still remains active, ToriPy closes admission,
...
is observed and logged as lingering; ToriPy does not wait indefinitely or run
the exit twice.
...
`NestApplication` is owned by `tori_py.application`; concrete adapters are owned
