...
Add the conventional `nestpy run` command and perform adversarial hardening of
...
```toml
[project.scripts]
nestpy = "nestpy.cli:main"
```

`nestpy run module:factory` is the only v1 serving command.
...
```text
The 'nestpy run' command requires the CLI extra.
Install it with: uv add 'nestpy[cli]'
```

Importing `nestpy`, `nestpy.core`, or `nestpy.cli` without executing `run` MUST
NOT import Uvicorn.
...
7. call the existing `nestpy.starlette.asgi` wrapper;
...
```text
uv run pytest packages/nestpy/benchmarks --benchmark-only
```
...
Examples live under root `examples/nestpy/`, never under framework source. At
...
N6 and Nestpy v1 are complete when CLI serving uses the exact production ASGI
