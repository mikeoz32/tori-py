...
- `CQRS_EVENT_SOURCING_IMPLEMENTATION_PLAN.md` is approved.
- The existing `cqrs-core` test suite and package boundary are stable.

## Deliverables

- Distribution `cqrs-event-sourcing` and import package `cqrs_event_sourcing`.
- Workspace membership, local `cqrs-core` dependency, package facade, and
  `py.typed` marker.
- Isolated import-boundary and wheel/source artifact checks.

## Invariants

- Dependency direction is `cqrs-event-sourcing -> cqrs-core` only.
- Runtime dependencies are `cqrs-core` and the Python standard library.
...
- Wheel and source distributions install with a local `cqrs-core` artifact.
