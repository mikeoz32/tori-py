...
- `Inject` uses normal Nestpy tokens and exact owner visibility.
...
- Message enhancer implementation classes must be explicit Nestpy providers;
...
- One application-wide registry fully describes every message entry point before
  a transport starts.
- Controllers require only normal Nestpy module registration and method
  decorators.
