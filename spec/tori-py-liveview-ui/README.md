...
3. The six helpers are stateless and return Python 3.14 `Template` values. Their
   ...
   class injection.
4. Helpers preserve LiveView t-string escaping and trusted composition rules.
   Internally generated button attributes use `attrs`; callers receive no
   arbitrary attributes or classes parameter.
5. `button` maps an optional event and positive component target to Opal's
   `data-opal-click` and `data-opal-target`; a target requires an event. It adds
   no client-side behavior and does not own event routing or state.
