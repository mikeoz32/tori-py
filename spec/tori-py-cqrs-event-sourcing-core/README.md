...
This directory governs the implemented optional `tori-py-cqrs-event-sourcing-core`
distribution.
...
The event-sourcing package depends on `tori-py-cqrs-core` without changing the completed
first CQRS slice. It is framework-neutral, persistence-adapter-neutral, and has
Python standard library runtime dependencies only beyond `tori-py-cqrs-core`.
...
1. Distribution name is `tori-py-cqrs-event-sourcing-core`; import name is
   `tori_py_cqrs_event_sourcing_core`.
2. Dependency direction is `tori-py-cqrs-event-sourcing-core -> tori-py-cqrs-core` with no reverse
   import.
