...
bounded shutdown as one lifecycle-managed ToriPy subsystem.
...
1. ToriPy compiles modules/providers/controllers.
...
5. ToriPy driver binding completes.
6. ToriPy opens work-scope admission.
...
5. preserves time for ToriPy scope cancellation and resource cleanup;
...
The ToriPy kernel then closes work admission and drains/cancels tracked scopes.
...
- `StarletteAdapter` and `MicroservicesModule` share one ToriPy lifecycle for a
...
- Shutdown has the same bounded ownership guarantees as ToriPy HTTP and CQRS.
