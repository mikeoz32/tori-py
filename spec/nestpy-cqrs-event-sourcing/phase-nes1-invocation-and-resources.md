...
- NES0, Nestpy N8, Nestpy CQRS C3, event-sourcing ES6, and CQRS7 pass.

## Deliverables

- Integration contract tests against exception-aware Nestpy resources, public
  CQRS invocation interceptors, typed UoW outcomes/operation leases, and
  same-bus reentrancy rejection.

## Nestpy Resource Invariants
...
- Cancellation during bounded cleanup remains observable and cleanup tasks are
  tracked according to existing Nestpy shutdown rules.

## CQRS Interceptor Invariants

- `CqrsInvocationContext` implements Nestpy `ExecutionContext` with
  `execution_kind == "cqrs"`.
- Context maps every `ExecutionContext` property exactly: application ID from the
  Nestpy kernel, string `module_id` label, `route_id=None`, `request_id=None`,
