...
This directory governs the framework-neutral `persistent-streams`
distribution. Its architecture and delivery order are recorded in
[`PERSISTENT_STREAMS_ARCHITECTURE.md`](../../PERSISTENT_STREAMS_ARCHITECTURE.md)
and
...
1. Distribution name is `persistent-streams`; import name is
   `persistent_streams`.
2. Runtime code depends only on the Python standard library.
3. The package never imports CQRS, Nestpy, RabbitMQ, SQLAlchemy, PostgreSQL,
   Pydantic, msgspec, or Kinker application code.
