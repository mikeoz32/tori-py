# ToriPy CQRS Event-Sourcing Specifications

This directory specifies the implemented optional `tori-py-cqrs-event-sourcing`
...
The integration consumes public ToriPy, `tori-py-cqrs`, `tori-py-cqrs-core`, and
`tori-py-cqrs-event-sourcing-core` contracts without creating reverse dependencies.
...
| --- | --- | --- | --- |
| NES0 | [Workspace and public contracts](phase-nes0-workspace-and-contracts.md) | Implemented ToriPy, CQRS, and event sourcing | Package boundary and frozen public API |
...
1. Distribution name is `tori-py-cqrs-event-sourcing`; import name is
...
3. The integration has no HTTP-driver, ORM, database-driver, broker, serializer,
   or DI dependency beyond ToriPy itself.
...
5. Domain aggregates do not import ToriPy integration decorators.
...
7. `aggregate_repository(RepositoryClass)` used in `Annotated` returns the normal
   ToriPy injection marker for that repository token.
...
14. Nested same-bus command dispatch is rejected before enqueue and never shares
    an outer UoW.
15. Dynamic modules follow exact ToriPy identity, visibility, scope, lifecycle,
...
3. Update upstream ToriPy, CQRS, or event-sourcing specifications when their
