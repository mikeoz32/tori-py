...
Create an installable typed Nestpy integration with frozen dependency, import,
facade, and artifact boundaries before runtime behavior is added.

## Deliverables

- `packages/nestpy-persistent-streams` as a uv workspace member.
- Runtime import `nestpy_persistent_streams` with `py.typed`.
- Runtime dependencies limited to `nestpy` and `persistent-streams`.
...
- Base import does not load RabbitMQ, `rstream`, `aio_pika`, Starlette,
  SQLAlchemy, CQRS, event sourcing, microservices, Alembic, or Kinker.
...
- Private Nestpy imports where a public contract exists.
