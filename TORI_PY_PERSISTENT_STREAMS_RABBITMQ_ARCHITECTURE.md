...
The proposed `tori-py-persistent-streams-rabbitmq` adapter maps RabbitMQ native Streams
and Super Streams to core `PersistentLog` contracts and exposes a separate
ToriPy configured-adapter factory/runtime.
...
- `RabbitMqStreamAdapterFactory` and its managed runtime implement ToriPy
  composition, callback handoff, lifecycle, readiness, pipeline-completion, and
  shutdown contracts and run ToriPy adapter lifecycle/execution conformance.
...
is isolated under `packages/tori-py-persistent-streams-rabbitmq/tests/feasibility`.
...
All core and ToriPy publication surfaces accept explicit `record_id`; configured
ToriPy publishers generate a UUID only when omitted. No accepted or indeterminate
...
decode, ToriPy pipeline, handler, work-scope cleanup, cursor persistence, or
...
partition. Cursor persistence happens only after ToriPy reports complete pipeline
...
If a revised architecture passes a new RPS0, release requires core and ToriPy conformance separately,
