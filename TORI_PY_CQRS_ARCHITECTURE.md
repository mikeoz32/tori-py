# ToriPy CQRS Integration Architecture

## Boundary

`tori-py-cqrs` is an optional, driver-neutral integration package:

```text
tori-py-cqrs -> tori_py, tori-py-cqrs-core
```

Neither `tori_py` nor `tori-py-cqrs-core` imports the bridge. The bridge does not depend
on Starlette, FastAPI, Pydantic, or a process-global registry.

The implemented `tori-py-cqrs-event-sourcing` package composes this bridge with the
...
`TORI_PY_CQRS_EVENT_SOURCING_ARCHITECTURE.md`.

## Composition

`CqrsModule.for_root()` returns one keyed ToriPy `DeferredModule`. Handler
classes are registered once as ordinary ToriPy providers. The
`tori_py_cqrs.command_handler`, `query_handler`, and `event_handler` decorators
compose framework-neutral `tori-py-cqrs-core` handler metadata with ToriPy injectable
metadata, so decorated classes may be listed directly in a module's `providers`.
During bootstrap the integration uses ToriPy `DiscoveryService` to inspect every
...
intentionally overridden providers. Their normal ToriPy
...
compilation validates handler visibility and preserves the canonical ToriPy
...
2. open one module-bound ToriPy work scope;
...
interceptor contract. The interceptor pipeline remains owned by `tori-py-cqrs`:

- each invocation context implements ToriPy `ExecutionContext` with CQRS message,
  envelope, handler kind, canonical provider, owner module, and scoped resolver;
- graph and handler interceptor tokens resolve through normal ToriPy visibility;
...
This extension does not add persistence behavior to `tori-py-cqrs`. Optional
packages may contribute interceptors while `tori-py-cqrs` continues to own only
dispatch composition and scope execution.

## Lifecycle

The CQRS runtime is an eager ToriPy singleton lifecycle participant.
...
5. allow normal ToriPy request admission.
...
drains tracked handler and observer tasks before ToriPy closes work admission.

Arbitrary nested dispatch cycles during shutdown are not guaranteed. Once a bus
starts stopping, new submissions to that bus are rejected by `tori-py-cqrs-core`.

## Ownership

ToriPy owns handler construction, scopes, managed resources, and cleanup.
`tori-py-cqrs-core` owns envelopes, routing, bus semantics, transports, task tracking,
and event failure observation. The bridge owns only explicit registration
mapping and lifecycle coordination.

Event failure hooks receive the configured ToriPy provider token or discovered
...
private factory markers back to ToriPy aliases. A future `tori-py-cqrs-core` provider-key
